...
int parse_args(
    int argc, char** argv, std::vector<std::string> &images,
    std::vector<std::string> &files, uint64_t *_mem, uint64_t *_page) {
    ...
    while(i < argc) {
        ...
        if(i < argc && (strcmp(argv[i], "-f") == 0 || strcmp(argv[i], "--file") == 0)) {
    ...
    for(int i = 0; i <number_of_files; i++) {
        files.push_back(argv[file_start + i]);
    }
