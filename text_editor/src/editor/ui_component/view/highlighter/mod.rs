...
impl Highlighter {
    pub fn new(matched_word: Option<String>, selected_match: Option<Location>, file_type: Option<FileType>) -> Self {
        Highlighter {
    ...
    fn create_syntax_highlighter(file_type: Option<FileType>) -> Option<Box<dyn SyntaxHighlighter>> {
     
        if let Some(file_type)  = file_type {
            match file_type {
                FileType::Rust => {
                    return Some(Box::new(
                        RustSyntaxHighlighter::new()
                    ))
                }
                _ => return None,
            }
    ...
    fn create_search_highlighter(matched_word: Option<String>, selected_match: Option<Location>) -> Option<Box<dyn SyntaxHighlighter>>{
        Some(Box::new(
            SearchHighlighter::new(matched_word, selected_match)
        ))
    }
